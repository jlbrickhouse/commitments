...
negation = ['ak-','chik-','ik-','kil-','ke-','kiloh-','heloh-','hachik-']
personal = ['-li','ish-','','il-','e-','iloh-','eho-','hash-'] #ish + sa means h disappears, subject pronouns
time = ['','-tuk','-tok','-achin']
...
per_definite = ['sia','chia','','pia','hvchia']#'hvpia we us ; final h if predicative
...
#exceptions
def exceptions(returnlist):
    corrected = []
    for i in returnlist:
        corrected.append(doubleycheck(i))
        corrected.append(amchange(i))
        corrected.append(sachange(i))
        corrected.append(moveli(i))
...
def amchange(returnable):
    print(am[0])
    if am[0] in returnable:
        if returnable.startswith(am[0]) != True:
            print('oh boy')
            #find am, change to sam

def sachange(returnable): #sa changes to si when before a or o
    if returnable[2] == 'a':
        returnable = returnable[0] + 'i' + returnable[2:]
    if returnable[2] == 'o':
        returnable = returnable[0] + 'o' + returnable[2:]
    return returnable

def moveli (returnable):
    if '-li' in returnable:
        #put at end before time marker
        returnable = returnable

def ishhashchange(returnable):
    a = 0
    #if before s, change to is- and has-

def ilohchange(returnable):
    a = 0
    #change iloh- to iho- before all consonants

def ilchange(returnable):
    a = 0
    #il- changes to i- before all consonants

def main():
    a = open('/Users/linabrixey/PycharmProjects/inprogress/dictionary','r').readlines()
    b = []

    print(ditransitive('VT, balili'))
    #broken
    #ona
    #ikbi
    #takchi
